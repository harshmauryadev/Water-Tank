  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-white/90 backdrop-blur-lg border-b border-blue-100 py-3 shadow-md shadow-blue-900/5'
    ...
    >
        ...
        <div className="flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col">
              <span className="text-xl font-extrabold tracking-tight text-slate-900 flex items-center gap-0.5">
                Tanki <span className="bg-gradient-to-r from-blue-600 to-cyan-500 bg-clip-text text-transparent">Meter</span>
              </span>
              <span className="text-[9px] text-slate-500 font-bold tracking-wider">Smart Tank. Smart Life.</span>
            </div>
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-7">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className="text-sm font-semibold text-slate-600 hover:text-blue-600 transition-all relative py-1"
              >
                {link.name}
              </Link>
            ))}
          ...
          <div className="md:hidden flex items-center">
            <button
              ...
              className="p-2 text-slate-700 hover:text-blue-600 rounded-lg hover:bg-slate-100 transition"
      ...
      {/* Mobile Menu Drawer */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-white/95 backdrop-blur-xl border-b border-slate-200 px-6 py-5 mt-2 space-y-3 shadow-xl">
          <nav className="flex flex-col space-y-2">
            {navLinks.map((link) => (
              <Link
                ...
                className="py-2 text-base font-semibold text-slate-700 hover:text-blue-600 transition-colors"
              >
                {link.name}
              </Link>
            ))}
